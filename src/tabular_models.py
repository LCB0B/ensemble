...
class LogisticRegressionMSELoss(LightningModule):
    """Logistic regression model with elastic net regularization and AUC monitoring."""
...
def objective_LR_MSELoss(
    ...
    model = LogisticRegressionMSELoss(
        input_dim=train_data.shape[1], alpha=alpha, l1_ratio=l1_ratio, lr=lr
    )

    model = torch.compile(model)
    # Initialize logging, early stopping, and checkpointing
    log_folder = FPATH.BASELINE_LOGS / "lr_logs" / experiment_name
    log_folder.mkdir(exist_ok=True, parents=True)
    checkpoint_folder = FPATH.CHECKPOINTS / "logistic_regression" / experiment_name
    checkpoint_folder.mkdir(exist_ok=True)
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        dirpath=checkpoint_folder,
        filename=f"{outcome_name}_best_trial_{trial.number}",
        save_top_k=1,
        mode="max",
    )
    trainer = pl.Trainer(
...
class MLPRegressor(LightningModule):
    """Feedforward neural network for binary classification with elastic-net regularization and AUC logging."""
    ...
    def __init__(
        ...
        Args:
            ...
            l1_ratio (float): Mix between L1 and L2 (0=L2, 1=L1).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Predicted probabilities of shape (batch_size,).
        """
        return self.model(x).squeeze(-1)

    def _elastic_net(self) -> torch.Tensor:
        """
        Compute elastic-net penalty over all linear layer weights.

        Returns:
            torch.Tensor: Elastic-net penalty scalar.
        """
        ...
        for m in self.model:
            if isinstance(m, nn.Linear):
                w = m.weight
                l1 += torch.norm(w, p=1)
                l2 += torch.norm(w, p=2) ** 2
        return self.alpha * (self.l1_ratio * l1 + (1 - self.l1_ratio) * l2)

    def training_step(self, batch, batch_idx):
        """
        Training step: compute BCE loss, elastic-net penalty, combined loss, and log all three.
        """
        x, y = batch
        y_hat = self(x)

        # Binary cross‐entropy
        bce = nn.functional.mse_loss(y_hat, y)
        self.log("train_mse", bce, on_step=True, on_epoch=True)

        # Elastic‐net penalty
        enet = self._elastic_net()
        self.log("train_enet", enet, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        """
        Validation step: compute and log BCE loss and AUC.
        """
        x, y = batch
        y_hat = self(x)
...
def objective_MLP(
    ...
    model = torch.compile(model)
    # Initialize logging, early stopping, and checkpointing
    log_folder = FPATH.BASELINE_LOGS / "mlp_logs" / experiment_name
    log_folder.mkdir(exist_ok=True, parents=True)
