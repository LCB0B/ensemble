...
from src.earlylife.src.chunking import write_dataset_to_parquet_in_batches
from src.earlylife.src.paths import FPATH, check_and_copy_file, copy_file_or_dir
from src.earlylife.src.utils import filter_parquet_by_person_ids_to_dataset
...
def bin_column(col_name: str, dataset: ds.Dataset, bins: int) -> Path:
    ...
    output_path = FPATH.TEMP_FILES / f"binned_{col_name}_temp.parquet"
...
def process_dataset(
    ...
    if df_remove_pre_date_events is not None:
        ...
        filtered_dataset = write_person_id_birthday_filtered_dataset(
            ...
            FPATH.TEMP_FILES / "process_dataset.parquet",
            date_col_name,
        )
    else:
