...
def censor_person(person, outcome_info, background):
    """Removes events after the censor date"""
    # Since data is sorted by abspos, we can just do binary search
    last_valid_idx = bisect.bisect_left(person["abspos"], outcome_info["censor"])
...
def get_n_grid_tokens(last_valid_date, first_abspos, grid_interval):
    """Returns the number of grid_tokens between first_abspos and last_valid_date with step_size grid_interval"""
    return math.ceil((last_valid_date - first_abspos) / (grid_interval))
