...
https://github.com/openai/gpt-2/blob/master/src.earlylife.src/model.py
2) huggingface/transformers PyTorch implementation:
https://github.com/huggingface/transformers/blob/main/src.earlylife.src/transformers/models/gpt2/modeling_gpt2.py
"""
...
from src.earlylife.src.encoder_blocks import Block, CumulativeProbabilityLayer
from src.earlylife.src.metrics import CustomROCS, FusedAccuracyAtK
from src.earlylife.src.time2vec import Time2Vec
from src.earlylife.src.utils import print_main
...
class PredictionFinetuneNanoEncoder(BaseNanoEncoder):
    ...
    def predict_step(self, batch, batch_idx):
        _, decoded_output = self.standard_step(batch, batch_idx, repad=True)
...
class RiskNanoEncoder(FinetuneNanoEncoder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.metrics = {
            "AUROC": AUROC(
                "multilabel" if num_follow_up > 1 else "binary",
                ignore_index=-100,
    ...
    def standard_step(
        ...
        grid_tokens = decoded_output[batch["grid_tokens"]].view(-1)
        grid_targets = batch["target"][batch["target"] != -100]

        loss = self.criterion(grid_tokens, grid_targets)

        decoded_output = torch.nn.utils.rnn.pad_sequence(
            [row[mask] for row, mask in zip(decoded_output, batch["grid_tokens"])],
            batch_first=True,
        )
