"""
Time Token version of GPT Language Model - uses discrete time tokens instead of Time2Vec.
Time information is embedded directly in token sequences as YEAR_XXXX and AGE_YY tokens.

Based on the original encoder_nano_risk.py but without Time2Vec temporal embeddings.
References:
...
class BaseNanoEncoder(pl.LightningModule):
    def __init__(self, *args, **kwargs):
        ...
        # Token embeddings (includes time tokens: YEAR_XXXX, AGE_YY)
    ...
    def embed_information(self, batch):
        """Embeds information using time tokens embedded in sequences"""
        # Time information is already embedded in the event tokens as YEAR_XXXX and AGE_YY
...
class GenerativeNanoEncoder(PretrainNanoEncoder):
    ...
    def _unpack_and_forward_generation(self, batch):
        ...
        # Create unpacked batch - only need event and segment for time tokens
        unpacked_batch = {
    ...
    def embed_information(self, batch):
        """
        Embeds token + segment features. Time information comes from time tokens in the sequence.
        """
        x = self.embedding(batch["event"])
        x += self.segment_embeddings(batch["segment"])
    ...
    def generate(
        ...
        for k in ["event", "segment", "attn_mask"]:
    ...
    def _extend_sequences(self, batch, next_tokens, current_length):
        ...
        # Segment information (simple continuation for time tokens)
        if current_length > 0:
