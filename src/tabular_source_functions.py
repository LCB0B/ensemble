...
from src.tabular_preprocessing import (  # noqa: E402
...
def preprocess_bef(
    ...
    for year in tqdm(years, "BEF: Iterating over years"):
...
def merge_parquet_outputs(base_output_folder: Path, n_groups: int) -> None:
    ...
    for i in range(n_groups):
        folder = base_output_folder.parent / f"{base_output_folder.name}_{i}"
        all_files.update(f.name for f in folder.glob("*.parquet"))
