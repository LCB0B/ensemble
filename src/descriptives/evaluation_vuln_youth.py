...
def plot_calibration_overall_1row(
    ...
    for i, outcome in enumerate(outcome_order):
        for j, calibrated in enumerate([False, True]):
            ...
            cal = "B) Calibrated" if calibrated else "A) Uncalibrated"
...
def plot_subgroup_levels_vertical_longdf(
    subgroup_df: pl.DataFrame,
    pred_cols: List[str],
    model_mapping: Dict[str, str],
    ...
    """
    Plots vertical errorbar chart of subgroup metrics with x-axis jitter by model.

    Args:
        subgroup_df (pl.DataFrame): Long-format df with one outcome and metric_type.
        pred_cols (List[str]): List of prediction column names.
        model_mapping (Dict[str, str]): Mapping of model col -> display name.
        ax (Optional[Axes]): Matplotlib axis. Creates one if None.
        jitter (float): Amount of horizontal jitter between models in the same subgroup.
        models (Optional[pd.Categorical]): Ordered categories of models to plot.
    ...
    # base positions for each subgroup
    subgroups = sorted(df_wide["subgroup"].unique())
    subgroup_to_x = {sg: i for i, sg in enumerate(subgroups)}
    df_wide["x_base"] = df_wide["subgroup"].map(subgroup_to_x)

    # prepare figure/axis
    if ax is None:
        fig, ax = plt.subplots(figsize=(max(6, len(subgroups) * 0.6), 5))
    ...
    # plot each model
    for i, model in enumerate(models):
        df_m = df_wide[df_wide["Model"] == model]
        x_jittered = df_m["x_base"] + model_offsets[i]
        y = df_m["metric"]
        yerr = [
            df_m["metric"] - df_m["lb"],
            df_m["ub"] - df_m["metric"],
    ...
    ax.set_xticklabels(subgroups, ha="center")
...
def plot_level_barplot(
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(df["Model"].values, rotation=0, ha="center")
    ax.set_ylabel(metric_label if metric_label else metric.upper())
    ax.set_xlabel("")
    ax.set_title("")
    ax.legend(title="Model", bbox_to_anchor=(1.05, 1), loc="upper left")
...
def censored_calibration_curve(
    y_true,
    y_prob,
    *,
    n_bins=5,
    ...
    """Own version of sklearn calibration_curve to censor to at least 5 obs"""
...
def plot_calibration_single_outcome(
    df: pl.DataFrame,
    pred_cols: List[str],
    target_col: str,
    subgroup_cols: List[str],
    ...
    """
    Plot calibration curves for one outcome across 8 panels:
    first row = uncalibrated, second row = calibrated;
    columns = 4 subgroup combinations.

    Args:
        df (pl.DataFrame): Table containing preds, calib, subgroups.
        pred_cols (List[str]): Base prediction column names (e.g.
            ["preds_pretrained", "preds_finetuned", ...]).
        target_col (str): Binary target column name (e.g. "target").
        subgroup_cols (List[str]): Two binary subgroup columns
            (e.g. ["female", "native"]).
        model_mapping (Dict[str, str]): Map from base pred to label.
        axs (List[List[plt.Axes]]): 2×4 axes grid.
        fig (plt.Figure): Matplotlib Figure.
        calibrated_suffix (str): Suffix on calibrated cols.
        n_bins (int): Number of bins for calibration_curve.
    """
    df_pd = df.to_pandas()
    subgroups = sorted(product(*[df_pd[c].unique() for c in subgroup_cols]))
    assert len(subgroups) == 8, "Need exactly 8 subgroup combos"
    names = [
        " & ".join(f"{c[0].upper()}={v}" for c, v in zip(subgroup_cols, sg))
        for sg in subgroups
    ]
    names = [name.replace("M", "E") for name in names]
    ...
    for row in range(2):
        for col in range(8):
            ...
            sg_vals = subgroups[col]
            title = names[col]

            mask = np.logical_and.reduce(
                [df_pd[c] == v for c, v in zip(subgroup_cols, sg_vals)]
            )
            df_sub = df_pd[mask]
            # choose preds
            cols = [
                f"{pc}{calibrated_suffix}" if calibrated else pc for pc in pred_cols
            ]
            ...
            for pc in cols:
                ...
                try:
                    ...
                    y_true, y_prob = censored_calibration_curve(
                        df_sub[target_col], df_sub[pc], n_bins=n_bins
                    )
                    label = model_mapping.get(pc.replace(calibrated_suffix, ""), pc)
                    ax.plot(y_prob, y_true, marker="o", label=label)
                except ValueError:
                    continue
