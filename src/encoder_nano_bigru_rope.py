...
class BaseNanoEncoder(pl.LightningModule):
    ...
    def embed_information(self, batch):
        ...
        # if (abspos := batch.get("abspos")) is not None:
        #     x += self.t2v_abspos(abspos) # abspos measured in hours

        # if (age := batch.get("age")) is not None:
        x += self.t2v_age(batch["age"])

        # if (segment := batch.get("segment")) is not None:
    ...
    def forward(self, batch: Dict[str, Any]):
        ...
        for block in self.transformer.h:
            x = block(
                x, attn_mask, sinusoidal_pos=sinusoidal_pos
            )  # Add sinusoidal_pos back in for rope
