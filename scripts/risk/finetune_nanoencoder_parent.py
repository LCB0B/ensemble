...
from src.datamodule2 import FamilyPredictFinetuneLifeLDM
from src.encoder_nano_risk import FamilyRiskPredictionFinetuneNanoEncoder
...
if __name__ == "__main__":
    ...
    # Load hparams
    with open(
        FPATH.CONFIGS / "risk" / "hparams_risk_finetune.yaml", "r", encoding="utf-8"
    ) as stream:
        hparams = yaml.safe_load(stream)
    run_id = f"{get_wandb_runid(FPATH.TB_LOGS / hparams['experiment_name'])}-{hparams['dir_path']}-{hparams['outcome'].split('_')[-1]}_predict"
    seed_everything(73)
    ...
    dm = FamilyPredictFinetuneLifeLDM(
    ...
    model = FamilyRiskPredictionFinetuneNanoEncoder(**hparams)
