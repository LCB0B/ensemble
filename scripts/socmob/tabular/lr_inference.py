...
from src.tabular_models import LogisticRegressionMSELoss, SparseDataset
...
def main(hparams: DictConfig) -> None:
    ...
    model = LogisticRegressionMSELoss.load_from_checkpoint(
    ...
    predictions = [p.unsqueeze(0) if p.dim() == 0 else p for p in predictions]
