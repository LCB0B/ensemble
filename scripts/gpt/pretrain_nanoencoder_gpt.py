...
from src.encoder_nano_gpt import PretrainNanoEncoder  # noqa: E402
...
if __name__ == "__main__":
    ...
    # Load hparams
    with open(
        FPATH.CONFIGS / "gpt" / "hparams_gpt_pretrain.yaml",
        "r",
        encoding="utf-8",
    ) as stream:
    ...
    print(f"Info: Ignoring tokens with prefixes {hparams['ignore_tokens']}")
    hparams["ignore_tokens"] = [
        v
        for k, v in dm.pipeline.vocab.items()
        if k.startswith(tuple(hparams["ignore_tokens"]))
    ]
